...
local function reset_view()
	mp.command("no-osd set video-pan-x 0; no-osd set video-pan-y 0; no-osd set video-zoom 0; no-osd set panscan 0; no-osd set video-unscaled no")
	scaling_mode = 1
end
...
local function handle_g()
	if pending_g then
		pending_g = false
		if pending_g_timer then
			pending_g_timer:kill()
			pending_g_timer = nil
		end
		goto_index(0)
		return
	...
	pending_g = true
	pending_g_timer = mp.add_timeout(0.7, function()
		pending_g = false
		pending_g_timer = nil
	end)
end
...
local bindings = {
	{ "q", "quit", function() mp.command("quit") end },
	{ "ESC", "quit-escape", function() mp.command("quit") end },
